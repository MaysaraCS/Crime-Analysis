...
const AdminInsertPage = ({ isOpen, onClose, onInserted }) => {
  const { token } = useAuth();
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const selectedMain = metaData[0]?.main_category ?? '';
        
        setForm({
          main_category: selectedMain,
          subcategories: [],
          neighbourhood_name: neighData[0]?.name ?? '',
          date: '',
          offender_income_level: 'middle',
          climate: 'moderate',
          time_of_year: 'summer',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/crime-form`,
        {
          method: 'POST',
          headers: {
      ...
      if (!res.ok) {
        throw new Error(data.detail || 'Failed to insert');
      }
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/30">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-3xl max-h-[90vh] overflow-y-auto p-6">
        <h3 className="text-xl font-semibold mb-4">Insert New Crime Record</h3>
        {loading ? (
...
export default AdminInsertPage;